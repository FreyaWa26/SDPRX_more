...
def initial_state(data1, data2,data3, M, N1, N2,N3, idx1_shared, idx2_shared, ld_boundaries1, ld_boundaries2, alpha=1.0, a0k=.5, b0k=.5):
    num_clusters = sum(M)
    ...
    state['idx_pop3'] = np.concatenate(tmp3)
...
def calc_b(j, state, ld_boundaries1, ld_boundaries2, ref_ld_mat1, ref_ld_mat2):
    ...
    start_i2 = ld_boundaries2[j][0]
    end_i2 = ld_boundaries2[j][1]
    start_i3 = ld_boundaries3[j][0]
    end_i3 = ld_boundaries3[j][1]
    ref_ld1 = ref_ld_mat1[j]
    ref_ld2 = ref_ld_mat2[j]
    ref_ld3 = ref_ld_mat3[j]
    shrink_ld1 = ref_ld1; shrink_ld2 = ref_ld2; shrink_ld3 = ref_ld3
    b1 = state['tau']*np.dot(state['A1'][j], state['X1_'][start_i1:end_i1]) - state['tau'] * \
    (np.dot(state['B1'][j], state['beta1'][start_i1:end_i1]) - np.diag(state['B1'][j])*state['beta1'][start_i1:end_i1])+ \
    2*np.dot(state['C'].transpose()[j], state['beta1'][start_i1:end_i1]) - np.diag(state['C'].transpose()[j])*state['beta1'][start_i1:end_i1])
    b2 = state['tau']*np.dot(state['A2'][j], state['X2_'][start_i2:end_i2]) - state['tau'] * \
    (np.dot(state['B2'][j], state['beta2'][start_i2:end_i2]) - np.diag(state['B2'][j])*state['beta2'][start_i2:end_i2])+ \
    2*np.dot(state['C'][j], state['beta2'][start_i2:end_i2]) - np.diag(state['C'][j])*state['beta2'][start_i2:end_i2])
    b3 = state['eta']*np.dot(state['A3'][j], state['beta_margin3_'][start_i3:end_i3]) - state['eta']**2 * \
    (np.dot(state['B3'][j], state['beta3'][start_i3:end_i3]) - np.diag(state['B3'][j])*state['beta3'][start_i3:end_i3])
    state['b1'][start_i1:end_i1] = b1
    state['b2'][start_i2:end_i2] = b2
    state['b3'][start_i3:end_i3] = b3
...
def sample_assignment(j, idx1_shared, idx2_shared, ld_boundaries1, ld_boundaries2, ref_ld_mat1, ref_ld_mat2, state, VS, rho):
    ...
    cluster_var = state['cluster_var'][idx]
    pi = np.array(state['pi'])[idx]
    ak1 = np.add.outer(.5*state['tau']*np.diag(B1[idx1_shared[j],:][:,idx1_shared[j]]), .5*(1-rho_3**2)/((det_sigma)*cluster_var))
    ak2 = np.add.outer(.5*state['tau']*np.diag(B2[idx2_shared[j],:][:,idx2_shared[j]]), .5*(1-rho_2**2)/((det_sigma)*cluster_var))
    ak3 = np.add.outer(.5*N3*state['eta']**2*np.diag(B3[idx2_shared[j],:][:,idx3_shared[j]]), .5*(1-rho_1**2)/((det_sigma)*cluster_var))
    ck1 = (rho_3-rho_1*rho_2) / (det_sigma*cluster_var)
    ck2 = (rho_2-rho_1*rho_3) / (det_sigma*cluster_var)
    ck3 = (rho_1-rho_2*rho_3) / (det_sigma*cluster_var)
